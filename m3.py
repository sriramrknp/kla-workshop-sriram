...
f1 = open("../Milestone_Input/Milestone 3/Source.txt","r")
f0 = open("../Milestone_Input/Milestone 3/POI.txt","r")
f2 = open("m3_out.txt","w")
...
for j in points_source.keys():
    if len(points_source[j]) == 9:
        ...
        print(angles_src[j])
        ang_push = True
        for i in range(0, len(angles_src[j])):
            if angles_src[j][i] != angles_mea[0][i]:
                ang_push = False
                break
        if ang_push == True:
            count.append(j)
